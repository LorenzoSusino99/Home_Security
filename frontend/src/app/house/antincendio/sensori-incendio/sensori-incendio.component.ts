...
import { SensorHistoryDialogComponent } from '../sensor-history-dialog/sensor-history-dialog.component';

@Component({
  selector: 'app-sensori-incendio',
  standalone: true,
  imports: [
    CommonModule,
    MatTableModule,
  ...
  templateUrl: './sensori-incendio.component.html',
  styleUrl: './sensori-incendio.component.css'
})
export class SensoriIncendioComponent implements OnInit {
  ...
  tableData() {
    this.sensorWs.getSensorUpdates().subscribe(data => {
      // Filtro solo i sensori antincendio (tipo gas)
      const gasSensors = data.filter(sensor => sensor.type === 'smoke');
      //console.log(gasSensors)
      this.source.data = gasSensors;
      this.total = gasSensors.length;
    });
  }
