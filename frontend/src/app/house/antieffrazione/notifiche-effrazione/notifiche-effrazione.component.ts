...
@Component({
  selector: 'app-notifiche-effrazione',
  standalone: true,
  imports: [
    CommonModule,
    MatTableModule,
  ...
  templateUrl: './notifiche-effrazione.component.html',
  styleUrl: './notifiche-effrazione.component.css'
})
export class NotificheEffrazioneComponent implements OnInit {
  ...
  loadInitialData() {
    ...
    this.houseService.getBurglarAlarms().subscribe({
      next: (res: any) => {
        console.log("Burglar Alarms: ", res)
        const burglarAlarms = (res as any[]).filter((alarm: any) => alarm.type === 'burglary');
        this.source.data = burglarAlarms;
        this.total = burglarAlarms.length;
  ...
  listenToRealTimeAlarms() {
    this.alarmsWs.getAlarmsUpdates().subscribe((data: any[]) => {
      ...
      console.log(data)
      const gasAlarms = data.filter(alarm => alarm.type === 'burglary');
